...
func setupCommands(s Server, tracker *SeriesTracker) []Command {
	var measurementN = 10
	var seriesN = 100
...
func TestServer_DELETE_DROP_SERIES_DROP_MEASUREMENT(t *testing.T) {
	//TODO: run too long. ingnore temporary
	return
	...
	tracker := NewSeriesTracker(r, s, db, rp)
	commands := setupCommands(s, tracker)
	for i := 0; i < N; i++ {
...
func NewSeriesTracker(r *rand.Rand, server Server, db, rp string) *SeriesTracker {
	...
	cli := coordinator.MetaClientImpl{Addrs: []string{raftmeta.NewConfig().MyAddr}}
	data, err := cli.Data()
	if err != nil {
		panic(err)
	}
